...
class Queue:
    ...
    def enqueue(self, value):
        ...
        """
        #self.queue.insert(0, value) should use append(value)
    ...
    def get_next_person(self):
        ...
        if self.people.is_empty():
            ...
            if person.turns > 1:  
                person.turns -= 1 
                self.people.enqueue(person)
            # add logic to check for 0 or less for infinite amount of enqueue()
            elif person.turns <= 0:
                self.people.enqueue(person)
            print(person.name)
...
# Defect(s) Found: The enqueue() uses insert instead of append() which causes it to stack.
...
for i in range(10):
    players.get_next_person()
#print(players)    
# Defect(s) Found: doesn't print Tim since he has 0 turns which is supposed to be infinite.
