...
def calc_factorial(n):
    """
    Calculate n! which equals n * (n-1) * (n-2) * ... * 2 * 1 -----O(N)
    """
    factorial = 1
...
def calc_stats(numbers):
    ...
    of all the numbers, and the average of the numbers. ------O(N)
    """
    total_sum = 0
    count = 0
...
def print_triangle(size):
    """
    Print a filled in isosceles  triangle to the screen. O(N^2)
    """
...
def display_letters_in_names(names): #O(N) because names outer is the O(n) but, inner is 3 times each outter. so really O(3N) drop constant.
    """
